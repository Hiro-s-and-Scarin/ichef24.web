...
import { Users, MessageCircle, Heart, Share2, Plus, X } from "lucide-react";
  ...
  const onSubmit = async (data: CreateCommunityPostFormData) => {
    try {
      const postData: CreateCommunityPostData = {
        ...
        difficulty_level: data.difficulty_level as
          | "Fácil"
          | "Intermediário"
          | "Avançado",
        image_url: data.image_url,
        recipe_tags: data.recipe_tags?.filter(
        ...
        <div className="max-w-7xl mx-auto space-y-8">
              ...
              <CardContent className="p-6">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  <div>
                    ...
                    >
                      <option value="Fácil">
                        {translateDynamicData.difficulty("easy", i18n.language)}
                      </option>
                      <option value="Intermediário">
                        {translateDynamicData.difficulty(
                          "medium",
                          i18n.language,
                        )}
                      </option>
                      <option value="Avançado">
                        {translateDynamicData.difficulty("hard", i18n.language)}
                      </option>
                    </select>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {posts.map((post) => (
              <Card
                key={post.id}
                className="bg-white/80 dark:bg-gray-800/80 border-gray-200 dark:border-gray-700/50 backdrop-blur-sm overflow-hidden h-full flex flex-col"
              >
                {post.image_url && (
