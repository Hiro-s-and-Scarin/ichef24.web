...
export function EditRecipeAIModal({
  isOpen,
  onClose,
  onSave,
  recipe,
}: EditRecipeAIModalProps) {
  ...
  const [modalState, setModalState] = useState<EditRecipeAIModalState>({
    ...
    chatMessages: [
      {
        type: "ai",
        message: t("ai.improve.message", {
          title: recipe?.title || t("common.your.recipe"),
        }),
  ...
  const generateWithAI = async () => {
    ...
    setTimeout(() => {
      const newGeneratedRecipe: GeneratedRecipe = {
        ...
        description:
          "Uma versão melhorada da sua receita com técnicas avançadas.",
        time: "30 minutos",
        servings: "4 pessoas",
        difficulty: "Médio",
        ingredients: [
