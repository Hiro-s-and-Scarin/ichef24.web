...
export const createRecipeSchema = yup.object({
  ...
  cooking_time: yup
    .number()
    .min(1, "Tempo de cozimento deve ser maior que 0")
    .max(480, "Tempo de cozimento deve ser menor que 8 horas"),
  servings: yup
    .number()
    .min(1, "Porções deve ser maior que 0")
    .max(20, "Porções deve ser menor que 20"),
  difficulty_level: yup
    .number()
    .min(1, "Nível de dificuldade deve ser entre 1 e 5")
    .max(5, "Nível de dificuldade deve ser entre 1 e 5"),
  cuisine_type: yup
    .string()
    .max(100, "Tipo de culinária deve ter no máximo 100 caracteres"),
