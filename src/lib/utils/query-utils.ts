...
export class QueryUtils {
  ...
  invalidateUserRecipes() {
    this.queryClient.invalidateQueries({ queryKey: queryKeys.recipes.history() })
    this.queryClient.invalidateQueries({ queryKey: queryKeys.recipes.favorites() })
  }

  invalidateUser(userId: string) {
    this.queryClient.invalidateQueries({ queryKey: queryKeys.users.detail(userId) })
    this.queryClient.invalidateQueries({ queryKey: queryKeys.users.preferences(userId) })
  }
  ...
  invalidateUserSubscription(userId: string) {
    this.queryClient.invalidateQueries({ queryKey: queryKeys.plans.active(userId) })
    this.queryClient.invalidateQueries({ queryKey: queryKeys.plans.billing(userId) })
  }
  ...
  prefetchRecipe(id: string) {
    return this.queryClient.prefetchQuery({
      ...
      queryFn: async () => {
        const { getRecipeById } = await import("@/network/actions/recipes/actionRecipes")
